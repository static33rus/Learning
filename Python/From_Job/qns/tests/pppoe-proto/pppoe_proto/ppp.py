...
def run_ppp_17(test, interface):
    ...
    print(
       "{0} [{1}]: establish pppoed and LCP -> send IPCP Configuration Request with Null IP and Null DNS"
       "-> recv IPCP Configure-Nak with IP addr and 192.168.10.100 primary dns and 192.168.10.200 secondary"
       "-> no reply".format(test, interface)
    )
    ...
    # send Configure-Request IPCP ("0.0.0.0" = first ip 0.0.0.0)
    configure_request_ipcp = create_packet_ipcp_config(dst, sessionid, 0x01, 0x01, "0.0.0.0", p_dns="0.0.0.0", s_dns="0.0.0.0")
    print(" -> IPCP Configure-Request")
    packets = send_and_recv(interface, configure_request_ipcp)
    ...
    option = find_ipcp_option(configure_nak_ipcp[PPP_IPCP], 0x81)  # 0x81 primary dns
    if option is None:
        print("    >>> IPCP Options do not have primary DNS server IP Address")
        return False

    dns = check_ipcp_option(option, "192.168.10.100")
    if dns is None:
        return False

    option = find_ipcp_option(configure_nak_ipcp[PPP_IPCP], 0x83)  # 0x83 secondary dns
    if option is None:
        print("    >>> IPCP Options do not have secondary DNS server IP Address")
        return False

    dns = check_ipcp_option(option, "192.168.10.200")
    if dns is None:
        return False
...
def register(testdict):
    ...
    testdict.update({'ppp_17': run_ppp_17})
